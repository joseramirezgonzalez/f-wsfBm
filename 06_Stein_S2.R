...
# =============================================================================
# FAMILIA 6: Stein S^(2), Cov(h)=sigma^2 (1+lambda |h|)^(-1)
...
read_ten_series <- function(path) {
  ...
  for (bat in 1:5) {
    j <- width * (bat - 1L) + 1L; d <- raw[, j:(j + 2L), drop = FALSE]
    ...
    for (coordinate in c("Latitude", "Longitude")) {
      ...
      out[[k]] <- list(bat = bat, coordinate = coordinate, t = d$time[-1L],
        y = x[-1L] - x[1L], n_original = length(x), n_used = length(x) - 1L)
    }
  }
  out
}
...
fit_one <- function(s) {
  ...
  opt <- optimize(objective, log(LAMBDA_BOUNDS), tol = 1e-10)
  lambda <- exp(opt$minimum); prof <- profile_scale_loglik(s$y, cov_stein_unit(s$t, lambda))
  k <- 2L
  data.frame(bat=s$bat, coordinate=s$coordinate, n_original=s$n_original,
    n_used=s$n_used, model="Stein_S2", k=k, logLik_max=prof$logLik,
    AIC=-2*prof$logLik+2*k, sigma2=prof$scale, lambda=lambda,
...
results <- do.call(rbind,lapply(series,function(s){
  cat(sprintf("Ajustando Stein S2: Bat %d, %s ...\n",s$bat,s$coordinate));fit_one(s)}))
results <- results[order(results$bat,match(results$coordinate,c("Latitude","Longitude"))),]
OUTPUT_FILE <- file.path(dirname(normalizePath(DATA_FILE)),"resultados_Stein_S2.csv")
