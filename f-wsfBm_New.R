...
#-----Covarianza fraccionario

cov_c_H<-function(T,n,h) {
    D<-T/n
    K<-matrix(rep(0,n*n),n,n)
    for(i in 1:n) {
        for(j in 1:i){
...
#-----Loglikelihood fraccionario------


#------Log-likelihood trajectorias velocidad--------#
                    
log_verosimilitude_practical_frac<-function(T,h,mu,index){
k<-length(mu)-1
n<-index[k]                
D<-T/n                     
K<-cov_c_H(T,n,h)
...
log_verosimilitude_practical_frac_2<-function(T,sigma,h,mu,index){
k<-length(mu)-1
n<-index[k]                
D<-T/n                     
K<-cov_c_H(T,n,h)
...
#---Practical case: Study of bats
ruta<-file.choose()
datos<-data.frame(read_excel(ruta,col_names = FALSE)) #--Data

number<-4 #---Number of bats
...
#------------Change to lon data---------------
log_vero_pract_frac<-function(x){
-log_verosimilitude_practical_frac(T,x,lat,index)
}



log_vero_pract_frac(0.5)

#------Profile Likelihood 

h_max_lon_frac<-optim(0.5,fn=log_vero_pract_frac, lower=0.01, upper=0.99,method="L-BFGS-B")$par
...
K<-cov_c_H(T,n,h_max_lon_frac)
M<-K[index,index]
sigma_max_lon_frac<-sqrt((t(as.vector(lat[2:k]-lat[1]))%*%solve(M)%*%as.vector(lat[2:k]-lat[1]) )[1,1]/(k-1))
...
max_OU<-log_verosimilitude_practical_OU_2(T,sigma_max_lon_frac,h_max_lon_frac,lat,index)
...
beta_max_lon<-optim(0.002,fn=log_vero_pract, lower=0.001, upper=0.05,method="L-BFGS-B")$par
...
beta_max_lat<-optim(0.05,fn=log_vero_pract, lower=0.02, upper=0.08,method="L-BFGS-B")$par
